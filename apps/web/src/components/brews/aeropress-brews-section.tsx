  ...
  const setDialedIn = useMutation(
    trpc.aeropressBrew.setDialedIn.mutationOptions({
      onSuccess: (_data, variables) => {
        ...
        if (variables.brewId) track('brew_dialed_in', { method: 'aeropress' })
      },
    }),
  )
...
// the aeropress levers — Grind · Dose · Water · Steep. Grinder, device, days off
// roast and notes are demoted to the expander (BrewDetails), reached on both the
// mobile card and the desktop sub-row (see ADR-0003). AeroPress has no water
// temp, so no method-specific `extra` field.
const columns = [
