...
describe('AeropressBrewsSection', () => {
  it('shows the empty state when there are no brews', () => {
    ...
    expect(screen.getByText(/No aeropress brews yet/i)).toBeTruthy()
  })
  ...
  it('links each row to its edit page and the header to the new-brew form', () => {
    ...
    const editAnchor = table
      ...
      .closest('a')
    expect(editAnchor?.getAttribute('href')).toBe('/aeropress/a1/edit')
    // Header action links to the log form.
    const logAnchor = screen
      .getByRole('button', { name: 'Log Brew' })
      .closest('a')
    expect(logAnchor?.getAttribute('href')).toBe('/aeropress/new')
  })
  ...
  it('deletes the brew when the confirmation dialog is confirmed', async () => {
    ...
    try {
      ...
      await waitFor(() => expect(fetchSpy).toHaveBeenCalled())
      const [url, init] = fetchSpy.mock.calls[0]
      expect(String(url)).toContain('aeropressBrew.delete')
      expect(String(init?.body ?? '')).toContain('a1')
    } finally {
      fetchSpy.mockRestore()
    }
  })
  ...
  it('expands a desktop row on click to reveal the brew detail', () => {
    ...
    expect(detail.getByText('tasted clean')).toBeTruthy()
    // AeroPress has no water temp — no method-specific expander field.
    expect(detail.queryByText('Water temp')).toBeNull()
