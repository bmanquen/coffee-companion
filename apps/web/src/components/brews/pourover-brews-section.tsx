  ...
  const setDialedIn = useMutation(
    trpc.pouroverBrew.setDialedIn.mutationOptions({
      onSuccess: (_data, variables) => {
        ...
        if (variables.brewId) track('brew_dialed_in', { method: 'pourover' })
      },
    }),
  )
...
// the pour over levers — Grind · Dose · Water · Brew. Grinder, device, water
// temp, days off roast and notes are demoted to the expander (BrewDetails),
// reached on both the mobile card and the desktop sub-row (see ADR-0003).
const columns = [
...
export function PouroverBrewsSection() {
  ...
  const deviceDialedIn = useDeviceDialedIn('pourover')
  ...
  const table = useReactTable({
    ...
    // reveals grinder, device, water temp, days off roast and notes via
